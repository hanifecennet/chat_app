...
class _ChatDetailState extends State<ChatDetail> 
{
    ...
    .then(
      (QuerySnapshot querySnapshot) async
      {
        ...
        else
        {
          ...
          .then((value) => 
          {
            chatDocId = value as String //sonraddan eklendi as string
          });
        }
      },
    )
  ...
  {
    if (msg == '') return;
    chats.doc(chatDocId).collection('messages').add({ //yeni collection olusturma
  ...
  Widget build(BuildContext context) 
  {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot)
      {
        ...
        {
          return Center(
            child: Text("Yolunda gitmeyen bir şeyler var"),
          );
        }
         ...
         var data;
          return CupertinoPageScaffold(
            navigationBar: CupertinoNavigationBar(
              ...
              middle: Text(friendName, style: TextStyle(fontSize: 22),),
                            ...
                            return Padding(
                              ...
                              child: ChatBubble(//
                                clipper: ChatBubbleClipper6(
                                  nipSize: 0,
                                  radius: 15,
                                  type: BubbleType.receiverBubble, //farkli
                              ),
                              alignment: getAlignment(data['uid'].toString()),
                                margin: EdgeInsets.only(top: 20),
                                backGroundColor: isSender(data['uid'].toString())
                                    ? Color.fromARGB(255, 178, 8, 193)
                                    : Color(0xffE7E7ED),
                                child: Container(
